...
parser.add_argument('-RA', action='store', dest='RA',
                    help='Store a constant value for the RA of object.')

parser.add_argument('-DEC', action='store', dest='DEC',
...
print('DEC = {0} degrees'.format(float(results.DEC)))
...
def take_data(obs_length, time_per_iter, dt, init_ra=None, init_dec=None):
    ...
    def calc_pos(jd):
        if init_ra is not None and init_dec is not None:
